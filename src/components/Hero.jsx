...
const GlobeCanvas = ({ className = '' }) => {
  ...
  const drawContinent = useCallback(
    (ctx, points, color, radius, rotY, cx, cy) => {
      if (points.length < 3) return;
      const rotRad = rotY * Math.PI / 180;
      const projected = points.map(([lon, lat]) => {
        const p = project(lon, lat, radius);
        return {
          x: p.x * Math.cos(rotRad) - p.z * Math.sin(rotRad),
          y: p.y,
          z: p.x * Math.sin(rotRad) + p.z * Math.cos(rotRad),
        };
      });
  ...
  const drawArcs = useCallback(
    (ctx, radius, rotY, time, cx, cy) => {
      ...
      pairs.forEach((pair, idx) => {
        ...
        const dotX =
          (1 - t) * (1 - t) * p1s.x + 2 * (1 - t) * t * midX + t * t * p2s.x;
        const dotY =
          (1 - t) * (1 - t) * p1s.y + 2 * (1 - t) * t * midY + t * t * p2s.y;
        const grad = ctx.createRadialGradient(dotX, dotY, 0, dotX, dotY, 10);
        grad.addColorStop(
          0,
          `rgba(255,215,100,${0.6 + 0.3 * Math.sin(time / 2000 + idx)})`
        );
...
export default function Hero() {
  ...
  const years = useCounter(19);
  const success = useCounter(90);
  const cases = useCounter(2000);
  const countries = useCounter(50);

  const stats = [
    { value: years, suffix: '+', label: 'Years Experience' },
    { value: success, suffix: '%', label: 'Visa Success Rate' },
    { value: cases, suffix: '+', label: 'Applications Approved' },
    { value: countries, suffix: '+', label: 'Countries Covered' },
  ];

  return (
    <section className="relative flex min-h-screen items-center overflow-hidden bg-[#020916] px-4 pb-20 pt-28 sm:px-6 sm:pt-36 lg:px-8 lg:pt-40">
      {/* ── Ambient glows ── */}
      <div className="pointer-events-none absolute right-0 top-0 h-[600px] w-[600px] rounded-full bg-gradient-to-br from-[#D4AF37] to-transparent opacity-20 mix-blend-screen blur-[160px] animate-pulse-glow" />
      <div className="pointer-events-none absolute -bottom-32 -left-32 h-[500px] w-[500px] rounded-full bg-gradient-to-tr from-[#1a2b4c] via-[#2a3b5c] to-transparent opacity-10 blur-[140px]" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(rgba(255,255,255,0.012)_1px,transparent_1px)] [background-size:32px_32px]" />
      <div className="pointer-events-none absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 800 400%22%3E%3Cpath d=%22M100,200 Q150,180 200,200 T300,180 T400,200 T500,180 T600,200 T700,180 L800,200 L800,300 L0,300 Z%22 fill=%22none%22 stroke=%22%23D4AF37%22 stroke-width=%221%22/%3E%3C/svg%3E')] bg-cover bg-center opacity-[0.03]" />

      <div className="relative z-10 mx-auto w-full max-w-7xl grid lg:grid-cols-12 gap-12 lg:gap-10 items-center">
        {/* ── Left column ── */}
        <div className="z-20 w-full text-left lg:col-span-7">
          {/* Badge */}
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#D4AF37]/30 bg-[#04152d]/80 px-4 py-2 shadow-[0_4px_20px_rgba(214,175,55,0.06)] backdrop-blur-md animate-fadeInUp">
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#D4AF37] opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-[#D4AF37]" />
            </span>
            <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-200 sm:text-[11px]">
              Trusted Since 2006 • Visa & Immigration Excellence
            </span>
          </div>

          {/* Headline */}
          <h1 className="mb-6 text-4xl font-extrabold leading-[1.15] tracking-tight text-white animate-fadeInUp-delay sm:text-5xl md:text-6xl lg:text-7xl">
            Visa, Immigration <br className="hidden sm:inline" />
            <span className="relative mt-1 inline-block bg-gradient-to-r from-[#D4AF37] via-[#F3E5AB] to-[#AA7C11] bg-[length:200%_auto] bg-clip-text font-black text-transparent animate-textShimmer">
              &amp; Study Abroad
            </span>
            <span className="mt-2 block text-2xl font-medium tracking-wide text-gray-400 sm:text-3xl md:text-4xl lg:text-5xl">
              Experts.
            </span>
          </h1>
          ...
          <div className="mb-8 max-w-xl">
            <p className="text-base font-light leading-relaxed text-gray-400 sm:text-lg">
              Empowering students, ambitious professionals, and families to
              transcend borders through impeccably tailored consultancy,
              corporate relocation, and international academic access paths.
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              {['Student Visas', 'Work Permits', 'Corporate Relocation', 'Study Abroad'].map(
                (label) => (
                  <span
                    key={label}
                    className="inline-flex items-center gap-2 rounded-full border border-[#D4AF37]/20 bg-[#D4AF37]/10 px-3 py-1.5 text-xs text-[#D4AF37]"
                  >
          ...
          <div className="mb-12 flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row">
            <a
              href="#consultation"
              className="cta-primary group relative z-10 inline-flex w-full items-center justify-center gap-3 rounded-md bg-gradient-to-r from-[#D4AF37] to-[#AA7C11] px-8 py-4 text-sm font-bold uppercase tracking-wide text-gray-950 no-underline shadow-[0_4px_30px_rgba(214,175,55,0.25)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_4px_50px_rgba(214,175,55,0.5)] sm:w-auto"
            >
            ...
            <a
              href="#services"
              className="group inline-flex w-full items-center justify-center gap-2 rounded-md border border-[#D4AF37]/40 px-6 py-4 text-sm font-medium tracking-wide text-[#D4AF37] no-underline transition-all duration-300 hover:-translate-y-0.5 hover:border-[#D4AF37] hover:bg-[#D4AF37]/10 sm:w-auto"
            >
          ...
          {/* Metrics */}
          <div className="grid grid-cols-2 gap-3 border-t border-white/5 pt-8 sm:grid-cols-4 sm:gap-4">
            {stats.map((stat, i) => (
              <div
                key={i}
                className="metric-card cursor-default rounded-xl border border-white/[0.03] bg-gradient-to-br from-white/[0.02] to-transparent p-3 transition-all duration-300 hover:border-[#D4AF37]/25 hover:bg-white/[0.05] sm:p-4"
              >
                <div className="mb-0.5 flex items-center gap-2">
                  <span className="metric-number bg-gradient-to-r from-white to-gray-300 text-2xl font-black transition-colors duration-300 sm:text-3xl">
                    {stat.value}
                    {stat.suffix}
                  </span>
                </div>
                <div className="metric-label text-[9px] font-bold uppercase tracking-[0.12em] text-gray-500 transition-colors duration-300 sm:text-[10px]">
                  {stat.label}
                </div>
              </div>
            ))}
        ...
        {/* ── Right column – Globe ── */}
        <div className="hidden lg:col-span-5 lg:flex lg:w-full lg:items-center lg:justify-center lg:py-6">
          <div className="group/globe relative flex h-[480px] w-[480px] items-center justify-center xl:h-[540px] xl:w-[540px] animate-fadeIn">
            {/* Ambient shadow */}
            <div className="pointer-events-none absolute inset-6 rounded-full bg-[#D4AF37]/5 opacity-40 blur-3xl transition-opacity duration-1000 group-hover/globe:opacity-70" />

            {/* Orbit rings */}
            <div className="pointer-events-none absolute h-[96%] w-[96%] rounded-full border border-[#D4AF37]/12 shadow-[inset_0_0_60px_rgba(214,175,55,0.02)] orbit-ring-1" />
            <div className="pointer-events-none absolute h-[76%] w-[76%] rounded-full border border-dashed border-white/8 orbit-ring-2" />
            <div className="pointer-events-none absolute h-[56%] w-[56%] rounded-full border border-white/4" />

            {/* Globe */}
            <div className="relative h-64 w-64 overflow-hidden rounded-full border border-[#D4AF37]/25 shadow-[0_0_80px_rgba(212,175,55,0.08),inset_0_0_50px_rgba(0,0,0,0.9)] backdrop-blur-xl transition-colors duration-700 group-hover/globe:border-[#D4AF37]/50 sm:h-72 sm:w-72 xl:h-80 xl:w-80">
              <GlobeCanvas className="h-full w-full" />
              <div className="pointer-events-none absolute inset-0 rounded-full shadow-[inset_-30px_-30px_70px_rgba(0,0,0,0.95),inset_15px_15px_40px_rgba(212,175,55,0.08)]" />
            </div>
            ...
            {MARKETS.map((country, index) => {
              ...
              return (
                <div
                  key={index}
                  className="animate-orbit absolute left-1/2 top-1/2 z-20 -ml-9 -mt-9"
                  style={orbitStyles}
                >
                  <div
                    className="animate-counter-rotate relative flex h-16 w-16 cursor-pointer select-none items-center justify-center rounded-full border border-white/10 bg-[#030f20]/90 text-2xl shadow-2xl backdrop-blur-md transition-all duration-300 hover:scale-110 hover:border-[#D4AF37] hover:shadow-[0_0_30px_rgba(212,175,55,0.3)] sm:h-18 sm:w-18 sm:text-3xl"
                    onMouseEnter={() => setHoveredCountry(index)}
                    onMouseLeave={() => setHoveredCountry(null)}
                  >
                    <span className="scale-105 transform drop-shadow-md">
                      {country.code === 'US' && '🇺🇸'}
                      {country.code === 'UK' && '🇬🇧'}
                      {country.code === 'CA' && '🇨🇦'}
                      {country.code === 'AU' && '🇦🇺'}
                    ...
                    {hoveredCountry === index && (
                      <div className="tooltip-arrow animate-fadeInUp pointer-events-none absolute bottom-full left-1/2 z-50 mb-3 -translate-x-1/2 whitespace-nowrap rounded border border-[#D4AF37]/40 bg-[#061833] px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white shadow-[0_4px_24px_rgba(0,0,0,0.7)] sm:text-[11px]">
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .animate-fadeInUp, .animate-fadeInUp-delay, .animate-fadeIn,
          .animate-textShimmer, .animate-pulse-glow, .animate-orbit,
          .animate-counter-rotate, .orbit-ring-1, .orbit-ring-2 {
